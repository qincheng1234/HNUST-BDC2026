...
FEATURE_EXPERIMENTS = frozenset({"A0", "A1", "A2", "A3"})
_ALPHA_158_BOUNDARY = "sma_5"
_A3_LEVEL_COLUMNS = frozenset(
    {
        "开盘",
        "收盘",
        "最高",
...
def normalize_feature_experiment(experiment):
    """Validate and normalize the feature-ablation experiment identifier."""
    normalized = str(experiment or "A0").strip().upper()
    if normalized not in FEATURE_EXPERIMENTS:
        allowed = ", ".join(sorted(FEATURE_EXPERIMENTS))
        raise ValueError(f"Unsupported FEATURE_EXPERIMENT={experiment!r}; expected one of {allowed}")
    return normalized


def select_feature_experiment_columns(feature_columns_map, source_feature_set, experiment="A0"):
    """Return the raw feature columns for a registered, reproducible ablation.

    A1 uses the compact 39-feature set. A2 removes the 39-feature extension
    from the combined schema. A3 keeps scale-free Alpha158 and technical
    signals while excluding price, volume, amount, and moving-average levels.
    Cross-sectional and market features are appended separately by
    :func:`build_model_feature_columns`.
    """
    experiment = normalize_feature_experiment(experiment)
    if source_feature_set not in feature_columns_map:
        raise KeyError(f"Unknown source feature set: {source_feature_set}")

    if experiment == "A0":
        return list(feature_columns_map[source_feature_set])
    if experiment == "A1":
        if "39" not in feature_columns_map:
            raise KeyError("Feature map must contain the 39-feature schema for A1")
        return list(feature_columns_map["39"])

    if "158+39" not in feature_columns_map:
        raise KeyError("Feature map must contain the 158+39 schema for A2/A3")
    combined_columns = list(feature_columns_map["158+39"])
    try:
        alpha158_end = combined_columns.index(_ALPHA_158_BOUNDARY)
    except ValueError as error:
        raise ValueError(
            f"158+39 schema is missing expected boundary {_ALPHA_158_BOUNDARY!r}"
        ) from error
    alpha158_columns = combined_columns[:alpha158_end]

    if experiment == "A2":
        return alpha158_columns
    return [column for column in combined_columns if column not in _A3_LEVEL_COLUMNS]
...
def create_labeled_ranking_dataset(
    ...
    """Create ranking samples from rows whose forward labels already exist."""
    prepared = data.copy()
    ...
    if not windows:
        raise ValueError("No labeled ranking samples are available for this period")

    window_frame = pd.DataFrame(
        windows,
        columns=["date", "instrument", "sequence", "target"],
    )
